...
LEAVE_REASON, VACATION_REASON, PENALTY_MENU, SELECT_PENALTY_TYPE, ENTER_PENALTY_DETAILS, SELECT_EMPLOYEE_FOR_PENALTY, CONFIRM_PENALTY, EDIT_PENALTY_AMOUNT, SELECT_PENALTY_TO_EDIT = range(9)
...
async def start_penalty_manager(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("✏️ تعديل عقوبة موجودة", callback_data="edit_penalty")],
        [InlineKeyboardButton("❌ إزالة عقوبة", callback_data="remove_penalty")],
        [InlineKeyboardButton("📋 عرض جميع العقوبات", callback_data="view_all_penalties")],
...
async def handle_penalty_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "add_penalty":
        ...
        await query.edit_message_text(
            "🔢 **إدخال معرف العقوبة**\n\n"
            "أدخل معرف العقوبة التي تريد تعديلها:\n\n"
            "📋 للحصول على معرف العقوبة، يمكنك:\n"
            "1. استخدام /list_penalties <رقم_الهاتف>\n"
            "2. استخدام /all_penalties\n"
            "3. البحث في سجل الموظف"
        )
        return SELECT_PENALTY_TO_EDIT
    
    elif data == "remove_penalty":
        await query.edit_message_text(
            "🗑️ **إزالة عقوبة**\n\n"
            "أدخل معرف العقوبة التي تريد إزالتها:\n\n"
            "⚠️ **ملاحظة:** الإزالة تعني تعطيل العقوبة وليس حذفها\n"
            "✅ يمكن استرجاعها لاحقاً إذا لزم الأمر"
        )
        context.user_data['penalty_action'] = 'remove'
        return SELECT_PENALTY_TO_EDIT
    
    elif data == "view_all_penalties":
        penalties = await get_all_active_penalties()
        await query.edit_message_text(penalties, parse_mode='Markdown')
        return ConversationHandler.END
...
async def process_custom_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """معالجة المدخلات المخصصة"""
    user_input = update.message.text
    
    if context.user_data.get('awaiting_input') == 'amount':
        try:
    ...
    elif context.user_data.get('awaiting_input') == 'ban_days':
        try:
...
async def select_penalty_to_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    penalty_id_input = update.message.text
    
    try:
        penalty_id = int(penalty_id_input)
        penalty = get_penalty_details(penalty_id)
        
        if not penalty:
            await update.message.reply_text(
                f"❌ لم يتم العثور على عقوبة بالمعرف: {penalty_id}\n\n"
                "يرجى التأكد من المعرف والمحاولة مرة أخرى."
            )
            return SELECT_PENALTY_TO_EDIT
        
        context.user_data['selected_penalty'] = penalty
        action = context.user_data.get('penalty_action', 'edit')
        
        if action == 'remove':
            # عرض تأكيد الإزالة
            keyboard = [
                [InlineKeyboardButton("✅ نعم، إزالة العقوبة", callback_data="confirm_remove")],
                [InlineKeyboardButton("❌ لا، إلغاء", callback_data="cancel_action")]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            
            await update.message.reply_text(
                f"🗑️ **تأكيد إزالة العقوبة**\n\n"
                f"🆔 المعرف: {penalty['id']}\n"
                ...
                f"⚠️ **هل أنت متأكد من إزالة هذه العقوبة؟**\n\n"
                f"💡 **ملاحظة:** الإزالة تعني تعطيل العقوبة فقط، ويمكن استرجاعها لاحقاً.",
                reply_markup=reply_markup
            )
            ...
            keyboard = [
                [InlineKeyboardButton("💰 تعديل المبلغ", callback_data="edit_amount")],
                [InlineKeyboardButton("🚫 تعديل حظر السجائر", callback_data="edit_ban_days")],
                [InlineKeyboardButton("📝 تعديل السبب", callback_data="edit_reason")],
        ...
        await update.message.reply_text(
            "❌ إدخال غير صالح. يرجى إدخال رقم معرف صحيح.\n"
            "أعد إدخال معرف العقوبة:"
        )
        return SELECT_PENALTY_TO_EDIT
...
async def handle_penalty_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "confirm_remove":
        # إزالة العقوبة (تعطيلها)
        result = update_penalty(penalty['id'], {'is_active': False, 'resolved_by': query.from_user.id})
        
        if result['success']:
            await query.edit_message_text(
                f"✅ **تم تعطيل العقوبة بنجاح!**\n\n"
                f"🆔 المعرف: {penalty['id']}\n"
                f"👤 الموظف: {penalty['full_name']}\n"
                f"📝 المخالفة: {penalty['penalty_name']}\n\n"
                f"👤 تمت الإزالة بواسطة: {query.from_user.first_name}"
            )
            ...
            if penalty['telegram_id'] and PENALTY_SETTINGS['notify_employee_on_penalty']:
                try:
                    ...
                    logger.error(f"Failed to notify employee about penalty removal: {e}")
        else:
            await query.edit_message_text(f"❌ خطأ في تعطيل العقوبة: {result.get('error', 'خطأ غير معروف')}")
    ...
    if updates:
        ...
        if result['success']:
            ...
            for field, value in updates.items():
                ...
                await update.message.reply_text(f"• {field_name}: {old_value} → {value}")
            
            await update.message.reply_text(f"👤 تم التعديل بواسطة: {update.message.from_user.first_name}")
        else:
...
async def get_all_active_penalties():
    ...
    try:
        ...
        message += "`/penalty_manager` ثم اختر 'تعديل عقوبة موجودة'"
...
🔹 **داخل مدير العقوبات:**
1. **إضافة عقوبة:** البحث → اختيار موظف → اختيار مخالفة → إدخال سبب → تأكيد
2. **تعديل عقوبة:** إدخال المعرف → اختيار التعديل → إدخال القيمة الجديدة
3. **إزالة عقوبة:** إدخال المعرف → تأكيد الإزالة
    ...
    penalty_conv_handler = ConversationHandler(
        ...
        states={
            ...
            CONFIRM_PENALTY: [
                ...
                MessageHandler(filters.TEXT & ~filters.COMMAND, process_custom_input),
                CallbackQueryHandler(confirm_penalty)
            ],
            SELECT_PENALTY_TO_EDIT: [MessageHandler(filters.TEXT & ~filters.COMMAND, select_penalty_to_edit)],
            EDIT_PENALTY_AMOUNT: [
